...
%bischof2023_filterbank Filterbank used in |bischof2023|
%   Usage: [sig_out,gt_out,fc] = bischof2023_filterbank(sig_in);
%          [sig_out,gt_out,fc] = bischof2023_filterbank(sig_in, N, len, fs, f_range);
%
%   Input parameters:
%        sig_in      : Audio signal. Size (*time* x *ears*).
%        N           : Optional filter order of the gammatone filters. Default: 4.
%        len         : Optional filter length (in samples). Default: 512 samples.
%        fs          : Optional sampling frequency (in Hz). Default: 44100 Hz.
%        f_range     : Optional upper and lower frequencies (in Hz) for
%                      the filterbank. Default: [10 15500] Hz. 
%
%   Output parameters:
%       sig_out     : Filtered *sig_in*. Size: (*time* *fcnum* *ears*).
%       gt_out      : Output of all Gammatone filters.
%       fc          : Center frequencies (in Hz) of the used filters.
%
%
%   `bischof2023_filterbank(..)` 
%   creates a Bark-scaled Gammatone filterbank and filters the input signal.
%   Corner frequencies of Bark scale are according to Zwicker (1961).
%
%   See also: bischof2023 exp_bischof2023
%
%
%   References: zwicker1961saf gunawan2004
