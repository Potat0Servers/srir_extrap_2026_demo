function [car_out, state] = lyon2024_carstep(x_in, CAR_coeffs, state)
%lyon2024_carstep One-sample update step of the filtering
%
%   Usage: [car_out, state] = lyon2024_carstep(x_in, CAR_coeffs, state);
%
%   Input parameters:
...
%
%   `lyon2024_carstep` updates the filter cascade state taking into account
...
if CAR_coeffs.linear
  ...
  nlf = 1 ./ (1 + (v * CAR_coeffs.velocity_scale + CAR_coeffs.v_offset) .^ 2 ); %was: nlf = lyon20XX_ohcnlf(v, CAR_coeffs);
end
