...
%                   The default consider broadband noise (requires source_fs). 
%                   Default: []
%         
%     'source_fs'   Sampling rate of the source. Default: 0 Hz
%
%     'dirs'        Specify custom spatial grid for templates provided as a
%                   barumerli2023_coordinates class.
% 
%   
...
%       
%     'source'            Use a mono sound source provided with the parameter
%                         source_ir to compute targets. 
%
%     'binaural_source'   Use a binaural sound source provided with the 
%                         parameter source_ir as target. 
%
%
%
...
%   References: barumerli2023 


if isoctave
    warning('barumerli2023_featureextraction is not supported by Octave');
    return;
end

    definput.import={'amt_cache', 'barumerli2023_featureextraction'};

    % accept templates' directions as barumerli2023_coordinates object
    definput.keyvals.dirs = [];

    [flags, kv]  = ltfatarghelper({}, definput, varargin);
    
    % forcing HRTF sampling rate - any different value would
    % corrupt the feature computation
    kv.fs = sofa_obj.Data.SamplingRate;

    % some checks... please if you need to change them be careful... 
    assert(~xor((flags.do_source | flags.do_binaural_source), ~isempty(kv.source_ir)), ...
    ...
    if flags.do_template || flags.do_all
        ...
        if ~strcmp(flags.feature_monaural, 'none')
            ...
            monaural_left = local_resamplefeatures(monaural_left, features.coords, kv);
            monaural_right = features.monaural(:, (pl/2+1):end);
            monaural_right = local_resamplefeatures(monaural_right, features.coords, kv);

            template.monaural = [monaural_left, monaural_right];
        end
        
        [template.itd, template.coords] = ...
            local_resamplefeatures(features.itd, features.coords, kv);
        template.ild = ...
            local_resamplefeatures(features.ild, features.coords, kv);
...
function [feature] = local_computefeatures(sofa_obj, coords, kv, flags)  
    ...
    if flags.do_target
        if flags.do_binaural_source
            assert(kv.source_fs == kv.fs, 'sampling rate mismatch')
            % match array dimensions to sofa
            if size(kv.source_ir, 3) == 1
                stimulus = zeros(1, 2, length(kv.source_ir));
                stimulus(1,:,:) = kv.source_ir';
            else
                assert(size(kv.source_ir, 2), 'the second dimension must have two channels (i.e. binaural)')
                stimulus = kv.source_ir;
            end
        else
            if ~isempty(kv.targ_az)
                coords_search = barumerli2023_coordinates([kv.targ_az, kv.targ_el, ones(size(kv.targ_el))], 'spherical');
