...
% input variables.
[EL, ERBWeights] = local_setupPBC(sizeA, freq);
...
if isempty(norm)
    ...
    [~, temp, jump] = local_calculatePBC(A, B, 0);
    ...
    for i = 2:maxIt
        ...
        [~, temp, ~] = local_calculatePBC(A, B, offset(i));
        temp_weighted = temp .* w;
...
[diff, avgDiff_ERB, ~] = local_calculatePBC(A, B, norm);

%%  INTERNAL FUNCTIONS
    function [EL, ERBWeights] = local_setupPBC(sizeA, freq)
        ...
        for l = 1:91
            ...
            [iso226SPL(1:29, l), iso226Freq(1:29, l)] = local_iso226(l-1);
    ...
    function [pDiff, avgPDiff_ERB, jump] = local_calculatePBC(A, B, norm)
...
function [spl, freq] = local_iso226(phon)
