...
%   Usage: [m] = barumerli2023('sofa_obj', sofa_obj);
...
%
%   Output parameters:
%     m       : Matrix as in |localizationerror| with the actual
%               and predicted directions. |barumerli2023_metrics| can 
%               be used for further analysis of *m*.
%     doa     : Structure with the fields: 
%
%               - *estimations*: Matrix. Size: (*target_num* x *num_exp* x *pos*)
%                 with *pos* being coordinates in the Cartesian coordinate system.
%
...
if isoctave
    warning('barumerli2023 is not supported by Octave');
    return;
end
...
% return m matrix only if target's actual direction is available
if isfield(target, 'coords') && ~isempty(target.coords)
