...
[X,Y,Z] = sphere;
surf(X*0.975,Y*0.975,Z*0.975,'FaceColor','white','EdgeColor','none');
...
scatter3(dirs(:,1),dirs(:,2),dirs(:,3),600,met.qe_perdir(:),'filled');
cmap=colormap(bone(400));
colormap(flipud(cmap))
caxis([0 40])
colorbar("southoutside")
scatter3(dirs(:,1),dirs(:,2),dirs(:,3),50,'k','Marker','x','LineWidth',1);
title('Quadrant Errors');
axis off

xPlane = [0 0 0 0]; yPlane =[-1.2,1.2,1.2,-1.2] ; zPlane = [-1.2,-1.2,1.2,1.2];
...
elseif strcmp(fig,'fig3')
    dt=[10,20,50,100,200,500];

    fig=figure;
    
    subplot(3,1,1)
    
    errorbar(dt-0.2,mean(met.mod.s.rmsL,2),std(met.mod.s.rmsL,[],2),'Color',c1,'LineWidth',lw); hold on;
    L(3)=plot(dt-0.2,mean(met.mod.s.rmsL,2),'.-','MarkerSize',15,'Color',c1,'LineWidth',lw);
    errorbar(dt+0.2,mean(met.mod.d.rmsL,2),std(met.mod.d.rmsL,[],2),'Color',c2,'LineWidth',lw); hold on;
    L(4)=plot(dt+0.2,mean(met.mod.d.rmsL,2),'.-','MarkerSize',15,'Color',c2,'LineWidth',lw); 
    ...
    L(1)=yline(mean(met.emp.s.rmsL),'--','Color',c1,'LineWidth',lw);
    L(2)=yline(mean(met.emp.d.rmsL),'--','Color',c2,'LineWidth',lw);
    ...
    legend(L,'BP','BA','MP','MA','Location','NorthEast')
    xlim([0 502])
    ylim([0,10])
    
    subplot(3,1,2)
    errorbar(dt-0.2,mean(met.mod.s.rmsP,2),std(met.mod.s.rmsP,[],2),'Color',c1,'LineWidth',lw); hold on;
    L(3)=plot(dt-0.2,mean(met.mod.s.rmsP,2),'.-','MarkerSize',15,'Color',c1,'LineWidth',lw);
    errorbar(dt+0.2,mean(met.mod.d.rmsP,2),std(met.mod.d.rmsP,[],2),'Color',c2,'LineWidth',lw); hold on;
    L(4)=plot(dt+0.2,mean(met.mod.d.rmsP,2),'.-','MarkerSize',15,'Color',c2,'LineWidth',lw); 
    ...
    L(1)=yline(mean(met.emp.s.rmsP),'--','Color',c1,'LineWidth',lw);
    L(2)=yline(mean(met.emp.d.rmsP),'--','Color',c2,'LineWidth',lw);
    ...
    subplot(3,1,3)
    errorbar(dt-0.2,mean(met.mod.s.querr,2),std(met.mod.s.querr,[],2),'Color',c1,'LineWidth',lw); hold on;
    L(3)=plot(dt-0.2,mean(met.mod.s.querr,2),'.-','MarkerSize',15,'Color',c1,'LineWidth',lw);
    errorbar(dt+0.2,mean(met.mod.d.querr,2),std(met.mod.d.querr,[],2),'Color',c2,'LineWidth',lw); hold on;
    L(4)=plot(dt+0.2,mean(met.mod.d.querr,2),'.-','MarkerSize',15,'Color',c2,'LineWidth',lw); 
    ...
    L(1)=yline(mean(met.emp.s.querr),'--','Color',c1,'LineWidth',lw);
    L(2)=yline(mean(met.emp.d.querr),'--','Color',c2,'LineWidth',lw);
...
elseif strcmp(fig,'fig4')
    ...
    L(3)=plot(itd-0.005,mean(met.mod.s.rmsL,2),'.-','MarkerSize',15,'Color',c1,'LineWidth',lw); hold on%,
    errorbar(itd,mean(met.mod.d.rmsL,2),std(met.mod.d.rmsL,[],2),'Color',c2,'LineWidth',lw); hold on;
    L(4)=plot(itd,mean(met.mod.d.rmsL,2),'.-','MarkerSize',15,'Color',c2,'LineWidth',lw); 
    L(1)=yline(mean(met.emp.s.rmsL),'--','Color',c1,'LineWidth',lw);
    L(2)=yline(mean(met.emp.d.rmsL),'--','Color',c2,'LineWidth',lw);
    ...
    rectangle('Position',[0.25 mean(met.emp.d.rmsL)-std(met.emp.d.rmsL) 2.8 2*std(met.emp.d.rmsL)],'FaceColor',[c2,0.1],'EdgeColor',c2)
        legend(L,'BP','BA','MP','MA','Location', 'east')
    ylabel('lateral RMSE [deg]')
    ylim([0,15]);
    xlim([0.25,3.05])
...
%     xlabel('duration (ms)')
    ...
    L(3)=plot(sig-0.02,mean(rmsL_s,2),'.-','MarkerSize',15,'Color',c1,'LineWidth',lw); hold on%,
    L(4)=plot(sig,mean(rmsL_d,2),'.-','MarkerSize',15,'Color',c2,'LineWidth',lw); 
    ylabel('lateral RMSE [deg]')
    L(1)=yline(mean(met.emp.s.rmsL),'--','Color',c1,'LineWidth',lw);
    L(2)=yline(mean(met.emp.d.rmsL),'--','Color',c2,'LineWidth',lw);
...
elseif strcmp(fig,'fig6') 
    ...
    fig=figure;
    subplot(2,1,1)
    L(3)=plot(pr-0.1,mean(met.mod.s.gain,2),'.-','MarkerSize',15,'Color',c1,'LineWidth',lw);hold on;
    L(1)=yline(mean(met.emp.s.gain),'--','Color',c1,'LineWidth',lw);
    L(4)=plot(pr,mean(met.mod.d.gain,2),'.-','MarkerSize',15,'Color',c2,'LineWidth',lw);
    L(2)=yline(mean(met.emp.d.gain),'--','Color',c2,'LineWidth',lw);
    ...
    legend(L,'BP','BA','MP','MA','Location','southeast')
