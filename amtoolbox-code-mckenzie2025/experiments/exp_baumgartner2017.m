...
%   #Requirements: M-Stats SOFA
%   #Author: Robert Baumgartner (2017), Acoustics Research Institute, Vienna, Austria
%   #Author: Michael Mihocic (2024): catching if Octave is used, and if table function is not available -> return warning
...
if exist('table','file') == 0
    warning([mfilename ' requires table function (Matlab).']);
    return;
end
...
%     Pext = {Pext,Pext};
    warning('off','AMT:baumgartner2017outdated');
    ...
    amt_disp();
    warning('on','AMT:baumgartner2017outdated'); 
