...
if flags.do_basic
    ...
    environment.state.parameters.angle.value = 30;
...
if flags.do_dynamic
    ...
    agent.state.parameters.lateral_gain.value = 2; % lateral overshoot  
...
if flags.do_variants
    ...
    options.sample.support = (0:2.2:44)';
    
    % create variants
    pars = frambi_parameters('set', agent.state.parameters, {'itd_uncertainty'}, 3e-5);
    ...
    variants{2,1} = frambi_parameters('set', pars, {'lateral_gain'}, 2);
    ...
    f = figure('Position',[0,0,329,324],'Units', 'pixels');
    ...
    legend({'Variant w/ lateral gain', ...
                ...
                'Actual angle', ...
                'Reported angle'}, 'FontSize',9, 'Position',[0.460015052541321 0.69649221513236 0.534954395971761 0.303493441452626]);
    xlim([0, 35]);
    yticks(0:.3:.6)
    yticklabels(0:.3:.6)
    xlabel('Lateral angle (deg)', 'FontSize',12, 'FontWeight','normal');
    ylabel('Response probability', 'FontSize',12, 'FontWeight','normal');
    exportgraphics(f,'Fig5.eps','ContentType','vector')
end
