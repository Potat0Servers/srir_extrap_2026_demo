...
%
%     'fig12a'    Reproduce Fig. 12a.
%
%     'fig12b'    Reproduce Fig. 12b.
%
%     'fig13'    Reproduce Fig. 13.
%
%     'fig14a'    Reproduce Fig. 14a.
%
%     'fig14b'    Reproduce Fig. 14b.
%
%     'fig15'    Reproduce Fig. 15.
%
%
%   `exp_osses2022(flags)` reproduces the figures from Osses et al. (2022), where
%   the following models are compared:
%
%     - #1: |dau1997|, model from Dau et al., (1997).
%
%     - #2: |zilany2014|, model from Zilany et al., (2014).
%
%     - #3: |verhulst2015|, model from Verhulst et al., (2015).
%
%     - #4: |verhulst2018|, model from Verhulst et al., (2018).
%
%     - #5: |bruce2018|, model from Bruce et al., (2018).
%
%     - #6: |king2019|, model from King et al., (2019).
%
%     - #7: |relanoiborra2019|, model from Relano-Iborra et al., (2019).
%
%     - #8: |osses2021|, model from Osses and Kohlrausch (2021). 
%
%   `exp_osses2022(..., 'models', models)` can be used to show data for 
%   specific models only. To this end, *models* is a vector selecting the models
%   by the index as show in the list above. For example if *models* is `[1 3 6]`
%   then the data for Dau et al., (1997), Verhulst et al., (2015), and King et al.,
%   (2019) will be shown only.
%
%
%   Additionally, the following flags can be specified: 
%
%     'redo'     Recomputes data for specified figure. Optional. Default: 
%                `'normal'` which uses the cached data if available.
%
%     'plot'     Plot the output of the experiment. This is the default.
%
%     'no_plot'  Doesn't plot, only return some data (see code).
%
%
%   Figures:
%   --------
...
%
%   References: dau1997mapI zilany2014 bruce2018 verhulst2015 verhulst2018a king2019 relanoiborra2019 osses2021 osses2022
%
...
%   #Author: Piotr Majdak (2024): documentation improvements, volatile display improvements for AMT 1.6.
            ...
            if bRun
                ...
                switch models{k}
                    ...
                    case 'zilany2014'
                        ...
                        for j = 1:N
                            [~,~,~,c1,c2] = zilany2014(insig_here(:,j),fs,f0,kv{:},afb_flags{:},'silent');
                            idxs = 1:length(insig_here); % to remove the zero padding
                            out = c1(:)+c2(:);
                            out = out(idxs);
                            out_all(:,end+1) = out;

                            [~,~,~,c1,c2] = zilany2014(insig_here(:,j),fs,f0_lo,kv{:},afb_flags{:},'silent');
                            out = c1(:)+c2(:);
                            out = out(idxs);
                            out_lo_all(:,end+1) = out;

                            [~,~,~,c1,c2] = zilany2014(insig_here(:,j),fs,f0_hi,kv{:},afb_flags{:},'silent');
                            out = c1(:)+c2(:);
                            out = out(idxs);
                            out_hi_all(:,end+1) = out;
                            ...
                            for kk = 1:length(fc_resp)
                                fc_here = fc_resp(kk);
                                if fc_here >= 125
                                    [~,~,~,c1,c2] = zilany2014(insig_here(:,j),fs,fc_here, kv{:},afb_flags{:},'silent');
                                    out = c1(:)+c2(:);
                                    out = out(idxs);
                                    out_tmp(:,end+1) = out;    
                                else
                    ...
                    case 'verhulst2015'
                        % The default afb_flag related to velocity is 'no_v'.
                        % For this specific figure we actually need 'v'. For
                        % this reason, we replace 'no_v' by 'v':
                        idx_here = find(strcmp(afb_flags,'no_v'));
                        if ~isempty(idx_here)
                            afb_flags{idx_here} = 'v'; % overwritting 'no_v' by 'v':
                        end
                        out_model = verhulst2015(insig_here,fs,fc_flags{:},afb_flags{:},noihc_flags{:},noan_flags{:},'silent');
                        for j = 1:N
                    ...
                    case 'verhulst2018'
                        ...
                        out_model = verhulst2018(insig_here,fs,fc_flags{:},afb_flags{:},noihc_flags{:},noan_flags{:},'silent');
                        for j = 1:N
                            out = out_model(j).v(:,idx);
                            out_all(:,end+1) = out;
            ...
            if bRun
                ...
                switch models{k}
                    ...
                    case 'zilany2014'
                        ...
                        for ii = 1:N_sigs
                            [~,~,~,c1,c2] = zilany2014(insigs(:,j),fs,fc_ref(ii),kv{:},afb_flags{:},'silent');
                            if ii == 1
                                L = size(insigs,1);
                            end
                    ...
                    case 'verhulst2015'
                        ...
                        %end
                        outs = verhulst2015(insigs(:,j),fs,fc_flags{:},afb_flags{:},'v',noihc_flags{:},noan_flags{:},'silent');
                        if j == 1
                            CF_here = outs.cf;
                        end
                        outsig = [];
                    ...
                    case 'verhulst2018'
                        ...
                        %end
                        outs = verhulst2018(insigs(:,j),fs,fc_flags{:},afb_flags{:},'v',noihc_flags{:},noan_flags{:},'silent');
                        if j == 1
                            CF_here = outs.cf;
                        end
                        outsig = [];
        ...
        switch models{k}
            ...
            case 'zilany2014'
                ...
                if bRun
                    ...
                    for j = 1:N_sigs
                        [~,~,vihc(:,j)] = zilany2014(insig(:,j),fs,f0(j),kv{:},afb_flags{:},'silent');
                    end
                    c = [];
                    c.vihc = vihc;
                    c.fs   = fs;
            ...
            case 'verhulst2015'
                ...
                if bRun
                    outsig = verhulst2015(insig,fs,fc_flags{:},afb_flags{:}, ...
                        ihc_flags{:},noan_flags{:},nomfb_flags{:},'silent');
                    for j = 1:N_sigs
        ...
        if bRun
            ...
            switch models{k}
                ...
                case 'zilany2014'
                    ...
                    [outsig,psth,~,~,~,~,out] = zilany2014(insig,fs,fc,kv{:},afb_flags{:},'silent');
                    fs_an = fs;
                    out_psth.psth = psth;
                ...
                case 'verhulst2015'
                    ...
                    out = verhulst2015(insig,fs,fc_flag,afb_flags{:},an_flags{:},'anfL','anfM','anfH',an_kv{:},nomfb_flags{:},'silent');
                ...
                case 'verhulst2018'
                    fc_flag = fc; % one frequency will be simulated only
                    out = verhulst2018(insig,fs,fc_flag,afb_flags{:},an_flags{:},an_kv{:},nomfb_flags{:},'silent');
                    outsig = out(1).an_summed/(numL + numM + numH);
                    fs_an = out(1).fs_an;
                ...
                case 'bruce2018'
                    ...
                    out = bruce2018(insig,fs,fc,kv{:},afb_flags{:},'silent');
            ...
            switch models{k}
                ...
                case 'zilany2014'
                    ...
                    for j = 1:N_lvls
                        ...
                        % out_psTH.psth_binwidth = psth_binwidth;
                        kv={'fiberType',4,'numH',numH,'numM',numM,'numL',numL,'nrep',nrep,'psth_binwidth',psth_binwidth};
                        [outsig,psth,~,~,~,~,out] = zilany2014(insig(:,j),fs,CF,kv{:},afb_flags{:}),'silent';
                ...
                case 'verhulst2018'
                    fc_flag = CF;
                    out = verhulst2018(insig,fs,fc_flag,afb_flags{:},an_flags{:},'anfL','anfM','anfH',an_kv{:},nomfb_flags{:},'silent');
                    for j = 1:N_lvls
                        % psTH =out(kk).an_summed/numTot;
                        meanAN_L(:,j) = out(j).anfL;
                        meanAN_M(:,j) = out(j).anfM;
                ...
                case 'bruce2018'
                    ...
                    for j = 1:N_lvls
                        out = bruce2018(insig(:,j),fs,CF,kv{:},afb_flags{:},'silent');
        ...
        switch models{k}
            ...
            case {'zilany2014'}
                if bRun
                    ...
                    [an_summed,out_psth.psth,~,~,~,~,out] = zilany2014(insig,fs,fc,kv{:},afb_flags{:},'silent');
                    fs_an = fs;
            ...
            case {'verhulst2018'}
                if bRun
                    ...
                    hear_profile = 'Flat00';
                    fc_kv = {'hearing_profile',hear_profile};
                    out = verhulst2018(insig,fs,fc_flags{:},fc_kv{:},afb_flags{:},'anfL','anfM','anfH',an_kv{:},an_flags{:},nomfb_flags{:},'silent');
                    fs_an = out.fs_abr;
        ...
        switch models{k}
            ...
            case 'zilany2014'
                if bRun
                    ...
                    out = zilany2014(insig,fs,fc_ref,kv{:},afb_flags{:},'silent');
            ...
            case 'verhulst2015'
                if bRun
                    out_tmp = verhulst2015(insig,fs,fc_flags{:},afb_flags{:},an_flags{:},an_kv{:},nomfb_flags{:},'silent');
            ...
            case 'verhulst2018'
                if bRun
                    out_tmp = verhulst2018(insig,fs,fc_flags{:},afb_flags{:},an_flags{:},an_kv{:},nomfb_flags{:},'silent');
            ...
            case 'bruce2018'
                ...
                if bRun
                    ...
                    for j = 1:length(fc_ref)
                        output = bruce2018(insig,fs,fc_ref(j),kv{:},afb_flags{:},'silent');
                        out(:,j) = output.psth;
                    end
                    dt = psth_binwidth;
                    fs_an = 1/dt;
        ...
        switch models{k}
            ...
            case 'zilany2014'
                if bRun
                    ...
                    for j = 1:N_signals
                        mean_rate = zilany2014(insig(:,j),fs,fc,kv{:},afb_flags{:},'silent');
            ...
            case 'verhulst2015'
                if bRun
                    ...
                    out = verhulst2015(insig,fs,fc_flags{:},afb_flags{:},ihc_flags{:},an_flags{:}, ...
                        kv.an_keyvals{:},mfb_flags{:},'silent'); % no_ihc just reduce the data load here
                    subfs = out(1).fs_abr;
            ...
            case 'verhulst2018'
                if bRun
                    ...
                    out = verhulst2018(insig,fs,fc_flags{:},afb_flags{:},ihc_flags{:},an_flags{:}, ...
                        kv.an_keyvals{:},mfb_flags{:},'no_ihc','no_an','no_cn','silent');
                    subfs = out(1).fs_abr;
            ...
            case 'bruce2018'
                if bRun
                    ...
                    for j = 1:N_signals
                        ...
                        out_tmp = bruce2018(insig(:,j),fs,fc,fc_kv{:},kv{:},afb_flags{:},'silent');
                        if j == 1
                            dt = psth_binwidth;
                            fs_an = 1/dt;
...
if flags.do_fig15
    ...
    for k = 1:N_models
        ...
        switch models{k}
            case 'dau1997'
                if bRun
                    amt_disp('Calculating dau1997...','volatile');
            ...
            case 'zilany2014'
                if bRun
                    ...
                    for j = 1:length(fcs)
                        amt_disp(sprintf('Calculating zilany2014: band %.0f of %.0f\n',j,length(fcs)),'volatile');
                        t_start = tic;
                        mean_rate = zilany2014(insig,fs,fcs(j),afb_flags{:},kv{:},'silent');
                        if j == 1
                            L = size(insig,1);
                        end
            ...
            case 'verhulst2015'
                if bRun
                    t_start = tic;
                    amt_disp('Calculating verhulst2015...','volatile');
                    out = verhulst2015(insig,fs,fc_flags{:},afb_flags{:},ihc_flags{:},an_flags{:}, ...
                                       an_keyvals{:},mfb_flags{:},'silent');
                    t_elapsed = toc(t_start);
            ...
            case 'verhulst2018'
                if bRun
                    t_start = tic;
                    amt_disp('Calculating verhulst2018...','volatile');
                    out = verhulst2018(insig,fs,fc_flags{:},afb_flags{:},ihc_flags{:},an_flags{:}, ...
                        kv.an_keyvals{:},mfb_flags{:},'no_ihc','no_an','no_cn','silent');
                    t_elapsed = toc(t_start);
            ...
            case 'bruce2018'
                if bRun
                    ...
                    for j = 1:length(fcs)
                        amt_disp(sprintf('Calculating bruce2018: band %.0f of %.0f\n',j,length(fcs)),'volatile');
                        t_start = tic;
                        %%% To measure computing time only:
                        out_tmp = bruce2018(insig,fs,fcs(j),afb_flags{:},kv_one_rep{:},'silent');
                        ic_out = carney2015(out_tmp.psth(1:L),BMF,subfs, mfb_keyvals{:});
                        t_elapsed = toc(t_start);

                        %%% To assess a reliable PSTH:
                        out_tmp = bruce2018(insig,fs,fcs(j),afb_flags{:},kv{:},'silent');
            ...
            case 'king2019'
                ...
                if bRun
                    ...
                    t_start = tic;
                    amt_disp('Calculating king2019...','volatile');
            ...
            case 'relanoiborra2019'
                ...
                if bRun
                    ...
                    t_start = tic;
                    amt_disp('Calculating relanoiborra2019...','volatile');
            ...
            case 'osses2021'
                if bRun
                    ...
                    t_start = tic;
                    amt_disp('Calculating osses2021...','volatile');
...
%             end
            amt_disp(sprintf('%s: # of channels: %.0f; # of bands with a BMF of 100 Hz: %.0f',pad(models{k},16),Nr_fc,num_cfs),'documentation');
            amt_disp(sprintf('                  Time elapsed=%.3f s (%.4f s per channel, %.0f channels)',t_elapsed(1,k),t_elapsed(3,k),t_elapsed(2,k)),'documentation');
            % END Time elapsed --------------------------------------------
