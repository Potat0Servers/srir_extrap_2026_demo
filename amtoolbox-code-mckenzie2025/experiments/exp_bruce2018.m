...
%exp_bruce2018 Reproduction of the auditory-nerve filterbank with improved synapse modeling
%
%   `exp_bruce2011(flags)` reproduces Figures from Bruce et
%   al. (2018).
%
%   The following flags can be specified;
%
%     'fig7b'    Reproduces Fig. 7, Panel b.
%
%     'fig8b'    Reproduces Fig. 8, Panel b.
%
...
%   #Author: Ian Bruce (2018): original implementation.
%   #Author: Clara Hollomey (2021): integration for the AMT. 
...
if flags.do_fig7b
    ...
    amt_disp(''); % start the volatile display
    for ii = 1:numstims
       pin=scaletodbspl(pin,stimdbs(ii));
       amt_disp(['Calculating stimulus #' int2str(ii) ' of ' int2str(numstims) '...'],'volatile');
    ...
    amt_disp(); % end the volatile display
...
if flags.do_fig8b
    ...
    amt_disp(); % end the volatile of bruce2018
...
if flags.do_fig10 % flags.do_analyticalmeanvarcounts
    ...
    output = bruce2018(pin, round(1/dt), CF, 'nrep', nrep, 'fsmod', Fs, ... % signal parameters
           ...
           'cohcs', cohc, 'cihcs', cihc, 'reptime', 2, 'silent'); % hearing loss parameters
