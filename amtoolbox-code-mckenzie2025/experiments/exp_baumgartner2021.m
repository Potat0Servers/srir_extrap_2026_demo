...
%exp_baumgartner2021 Reproduction of the sound externalization based on multiple static cues
...
%              models (open symbols):
% 
%              - Exp. I: Effects of low-frequency with the IID set to zero (Hartmann and 
%                Wittenberg, 1996). IID set to zero (bilateral average 
%                magnitude); actual data from their Fig.7, average from N=2. 
%
%              - Exp. II: Effects of low-frequency with ipsilateral magnitudes flattened
%                (Hartmann and Wittenberg, 1996). IID compensated by 
%                contralateral magnitudes; actual data from their Fig.8, average 
%                from N=4. Simulated results for various cues, average from N=21. 
%              
%              - Exp.III: Effect of spectral smoothing of low-frequency sounds 
%                (Hassager et al., 2016). Sound were presented from two azimuths  
%                (left: 0; right: 50); actual data represents direct-sound condition from 
%                average from N=7. Simulated N=21. 
%
%              - Exp.IV: Effect of spectral smoothing in high frequencies 
%                (Baumgartner et al., 2017). Smooting represented by the spectral contrast 
%                (C=1: natural listener-specific 
...
%
%              - Exp.V: Effects of stimulus bandwidth and microphone casing (Boyd et al., 2012). 
%                Data for various mixes between simulations based on listener-specific 
%                BRIRs (100%) and time-delay stereophony (0%); N=3 (actual and simulated). 
...
%
%   References: boyd2012 hartmann1996 hassager2016 baumgartner2017looming
%
%   See also: baumgartner2021

%   #Author: Robert Baumgartner (2021): original implementation for the AMT.
  ...
  if isempty(cues)
    ...
    for isubj = 1:length(data)
      ...
      for iazi = 1:length(azi)
        ...
        for iB = 1:length(B)
          amt_disp(['Subject #' num2str(isubj) '/' num2str(length(data)) ...
                    ', Position: #' num2str(iazi) '/' num2str(length(azi)) ...
                    ', Band: #' num2str(iB) '/' num2str(length(B))],'volatile');
          if isnan(B(iB))
            target = template;
          else
