...
%   #Author: Alejandro Osses (2023): Adding prepanning inside the function relanoiborra2019
...
for q=1:Nsentences
    ...
    cal_factor = Pref*(1/rms(speech))*10^((speechSPL)/20);
    speech = cal_factor*speech; % Set speech level
    N = length(speech);  % Overall speech size
    
    for n=1:length(SNRs)
        
        noise = amt_load('relanoiborra2019', noise_name); % This is a pregenerated noise
        ...
        cal_factor = Pref*(1/rms(noise))*10^((speechSPL-SNRs(n))/20);
        noise = cal_factor*noise; % sets the level of the noise signal
        
        if size(noise) ~= size(speech)
            noise = noise'; % Flips noise signal if needed
        end
        
        test = noise + speech; % Generating the noisy mixture
        ...
        % Call model:
        flags4model = {'N_org',N_org, 'subject',sbj};
        tmp = relanoiborra2019(speech, test, fsRef, flow, fhigh, flags4model{:});
