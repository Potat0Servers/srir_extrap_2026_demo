...
%   #Author: Piotr Majdak (2023): adapted for various python evironments based on amt_configuration
...
switch environment
  case 'Python'
    [~,kv]=amt_configuration; % get the command to run python
    ...
    if ~isempty(input)
      ...
      % call
    [stat,res]=system([kv.pythoncmd ' ' module]);
      % check the status
    status.status=stat;
    status.res=res;
    if stat ~= 0
