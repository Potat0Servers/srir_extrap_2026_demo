...
%   #Author: Piotr Majdak (2023): expanded to work with all files, not models only. 
...
% mlock;
  ...
  if strcmp(lower(varargin), 'once')
    ...
    fileList = dir(fullfile(amt_basepath, '**', instring));
    ...
    elseif isempty(fileList)
      ...
      S = fileread(fullfile(fileList.folder,fileList.name));
    end
...
%read the information associated to the anchors
 ...
 %display that information 
  ...
  amt_disp(strrep(T, char([13 10]), newline));
  if ~strcmpi(varargin, 'once')
    amt_disp(' ');
