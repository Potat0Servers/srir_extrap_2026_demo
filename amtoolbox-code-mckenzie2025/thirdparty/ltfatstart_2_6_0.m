function ltfatstart_2_6_0(varargin)
%LTFATSTART   Start the LTFAT toolbox (adapted for the AMT)
%   Usage:  ltfatstart;
%
...
%   AUTHOR : Peter L. Søndergaard, Zdenek Prusa, Piotr Majdak
%   TESTING: NA
...
basedir = {filesep};
adp=''; % PM for AMT: initialize to save all paths
mods=local_ltfatmodules; % PM for AMT: get the list of modules
while ~isempty(d)
  for ii=1:length(d{1})
    name=d{1}(ii).name;
    ...
    %addpath([bp,basedir{1},name]); % PM for AMT: we do it at once after the loop
    ...
    if any(iffound)
        ...
        try
            idx=find(strcmp(mods(:,1),initfilename)); % PM for AMT: find the init file in mods
            if ~isempty(idx), status=mods{idx,2}; end % PM for AMT: set the status withouth calling the function
        catch
            % If the run command breaks, it might not cd back to the
            % original directory. We do it manually here:
            % cd(currdir); % PM for AMT: not required
        end
    end
    ...
    if status>0
      ...
      adp=[adp ',''' bp basedir{1} name ''''];  % PM for AMT: we save all the paths
    else
      % Something failed, restore the path
      % rmpath([bp,basedir{1},name]); % PM for AMT: not required
    end;
  end;
...
eval(['addpath(' adp(2:end) ');']); % PM for AMT: add all paths at once, this is faster than addpath in a loop
...
if ltfatstartprint
  ...
  banner = sprintf(['LTFAT version %s (for AMT). Copyright 2005-2023 Peter L. Søndergaard. ' ...
...
%     cd(currdir); % PM for AMT: not required, we are not changing the path
end
...
function mods=local_ltfatmodules % PM for AMT: store all modules and their status
mods={
  % init_file_name, status
  'auditoryinit.m', 1; ...
  % 'blockprocinit.m', 0; ... % not loaded with nojava
  'compinit.m', 2; ...
  'demosinit.m', 1; ...
  'deprecatedinit.m', 1; ...
  'filterbankinit.m', 1; ...
...
if isoctave
  mods{end+1,1}= 'octinit.m';
  mods{end,2}=2;
else
  mods{end+1,1}= 'mexinit.m';
  mods{end,2}=2;
end
