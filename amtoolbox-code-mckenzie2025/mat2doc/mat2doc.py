...
# Mat2doc: Documentation generator adapted for the AMT.
# 
# Created by Peter L. Soendergaard (2010-2013), see https://github.com/ltfat/mat2doc
# Changes by Zdenek Prusak (2015-2018): 
#   - convertion to Python 3
#   - many functional additions
# Changes by allthatsounds (2023): 
#   - --no-plot flag added. Use --no-plot to not execute Matlab when using the PHP target
# Integrated in the AMT by Piotr Majdak (2023, mat2doc 1.0): 
#   - Integration in the AMT, fork of https://github.com/ltfat/mat2doc
#   - Added the ignore list when processing MAT. Place filenames to be ignored in the file mat2doc/nodocs 
#   - Added version number and splash message
# Changes by Piotr Majdak (2023, mat2doc 1.1): 
#   - The file 'ignore-files' can be used to provide file names (with extension) to be excluded from mat2doc process.
#   - The file 'ignore-folders' can be used to provide directories to be excluded from mat2doc process.
#   - The previously untested 'nodocs' functionality:
#     - renamed to 'nodocs-files'. 
#     - Use the file 'nodocs-files' to provide file names of files to be copied only, not processed.
#     - Still untested because not used within the AMT.
#  - '$version' in urlbase (defined in conf.py) is replaced by the version string (read in the file defined by versionfile in conf.py)
#  - Improved error handling in sections "Input parameters" and "Output parameters"
#  - In PHP target, all section names have ":" at the end
# Changes by Piotr Majdak (2023, for AMT 1.5):
#  - When parsing "changes.m", no description is inserted (Issue #258)
#  - Bug fix: Searching for figures model_X --> model_X_ (Issue #273)
#  - When executing code (:::), the code, figures, and code output are now placed in a list table (class: figuretable)
#  - flag 'version' removed because mat2doc is now part of the AMT and thus versioned, and it was in conflict
#    with the '$version' read from urlbase. 
# Changes by Piotr Majdak (2024, for AMT 1.6):
...
#  - Keyphrases such "Input parameters", "Usage" or "See also" require a colon at the end. Otherwise these phrases cannot be used elsewhere in text
#  - In lines not beginning with three spaces: The problematic line is displayed now. 
#  - In contents.m files, the intend within each line is preserved now.
...
# Find all the Contents.m files, excluding directories beginning with those in ignoredirs
def find_indexfiles(projectdir, ignoredirs):
    indexfiles=[]
    ignoredirs=[s.replace('\r', '') for s in ignoredirs]
    print('Building index of all Contents.m files, ignoring: ',ignoredirs)
   
    for root, dirs, files in os.walk(projectdir, topdown=False):
        if 'Contents.m' in files:
            rel=os.path.relpath(root,projectdir)
            #print(rel)
            if rel.startswith(tuple(ignoredirs)):
                print('  Ignoring '+rel)
            else:
                for name in files:
                    if name=='Contents.m':
...
def build_ignoredocrelist(projectdir, ignore_file):
    ...
    indexfiles = []
    print('Nodocs: Files copied, not processed based on list from ', ignore_file)
    for root, dirs, files in os.walk(projectdir, topdown=False):
        for name in files:
            if name.split('.')[-1] != 'm':
                continue
            ...
            if not ok:
                ...
                print(s)
                indexfiles.append(s)
    print('  The following files will be copied, not processed: ', indexfiles)
    print('-----------------')
    return indexfiles
...
class GlobalConf(ConfType):
    def __init__(self,confdir,projectname,projectdir,outputdir=''):
        ...
        s=os.path.join(self.confdir,'ignore-files')

        if os.path.exists(s):
            self.ignorelist=safereadlines(s)
            self.ignorelist=[s.replace('\r', '') for s in self.ignorelist]
        else:
            self.ignorelist=[]

        #s=os.path.join(self.confdir,'nodocs') # PM: if 'nodocs' exists, it overwrites the list from 'ignore'. Why?
        #if os.path.exists(s):
        #    self.ignorelist=safereadlines(s)
        #else:
        #    self.ignorelist=[]                   
        ...
        print('  Project directory: '+self.projectdir)
        print('  Configuration read from: '+self.confdir)
        print('  Files to be ignored: ', self.ignorelist)
...
class TargetConf(ConfType):
    ...
    def __init__(self,g):
        ...
        self.urlbase=self.urlbase.replace('$version',g.version)
...
class ExecPrinter(BasePrinter):
    ...
    def parse(self):
        ...
        self.title=out['name'] #+' <br> '+out['description']
        ...
        out['body'].append('.. |nbspc| unicode:: U+00A0 .. non-breaking space')
        out['body'].append('.. role:: raw-html(raw)')
        out['body'].append('    :format: html')
        out['body'].append('')
        
        # Add the title
        s=out['name'] + ' :raw-html:`<br />` ' + out['description']
        ...
        for ii in range(len(buf)):
            ...
            if len(buf[ii].strip())>1:
                if not (buf[ii][0:3]=='   '):
                    print('   In function %s: This line does not start with three empty spaces.' % out['name'])
                    print(buf[ii]); 
                    sys.exit()
                else:
                    buf[ii]=buf[ii][3:]
        ...
        while len(buf)>0:
            ...
            if 'Input parameters:' in line:
                ...
                while not (len(line.strip())>0 and find_indent(line)==0):
                    ...
                    if len(buf):
                        ...
                        print('ERROR in section "Input parameters": Unexpected end of the section')
                        break
            ...
            if 'Output parameters:' in line:
                ...
                while not (len(line.strip())>0 and find_indent(line)==0):
                    ...
                    if len(buf):
                        ...
                        print('ERROR in section "Output parameters": Unexpected end of the section')
                        break
            ...
            if ':::' in line:
                ...
                out['body'].append('  ')                
                # We use the list table to bind the code, the image, and potential code output
                out['body'].append('.. list-table::')
                out['body'].append('  :class: figuretable')
                out['body'].append('        ')
                ...
                codebuf=[]
                # Append the code: the code is a literal block in a row of the list table
                out['body'].append('  * - .. code::')
                out['body'].append('         ')
                while not ((len(buf)==0) or (len(line.strip())>0 and find_indent(line)==0)):
                    codebuf.append(line)
                    if len(line.lstrip())>0:
                        out['body'].append('         ' + line.lstrip() + '')
                    line=buf.pop().rstrip()
                ...
                # Append the plots: each image is a row in the list table
                for i in range(nfigs):
                    if args.no_execplot==3:
                        if len(outbuf)>0:
                            out['body'].append('  * - This figure is from a previous version!')
                ...
                # Append the code output as a literal block as a row in the list table
                if args.no_execplot!=3:
                    if len(outbuf)>0:
            ...
            if not header_added:
                ...
                if not line[0]==' ': # First real line encountered
                    header_added=1
                    # print(out['name'])
                    if out['name']!='CHANGES': # No description for Changes.m
    ...
    def print_code_html(self):
        ...
        maincontents.append('<h1 class="title">'+self.parsed['name']+' <br> '+self.parsed['description']+'</h1>')
    ...
    def print_body(self,obuf):
        ...
        # If rst-debug flag active, print the raw reStructuredText input and exit
...
class ContentsPrinter(BasePrinter):
    ...
    def parse(self):
        ...
        while len(buf)>0:
            ...
            if (sep in line) and not (isnewblock(line)):
                ...
                obuf.append(['text',line[3:]]) # preserve the intend in a line
    ...
    def write_the_file(self):
        ...
        if self.c.t.basetype=='php' or self.c.t.basetype=='html':
            ...
            if self.c.g.rst_debug: #PM heute
                print(rststr)
                sys.exit()
    ...
    while len(buf)>0:
        ...
        if ind==-1:
            ...
            if ind+1==len(line.rstrip()):
                ...
                out.append((line[0:ind].strip(),sep,line[ind+3:].strip()))
    return out
...
#            print('Figures found!')
    ...
    """
        ...
        #print(output)
        #output='MARKER\nDummy placeholder'    # this simulates matlab
        ...
        #output=output[pos+6:].lstrip()
        output=output[pos+7:]
        #print("Output after .strip ********")
        #print(output)
    ...
    # _output file    
    nfigs=len([x for x in p if x[0:len(funname)+1]==(funname + '_')])-1

    if do_it:
        print(funname + '_: Created %i plot(s)' % nfigs)
    else:
        print(funname + '_: Found %i plot(s)' % nfigs)
...
def matfile_factory(conf,fname,targfname=''):
    ...
    if buf[0].split()[0]=='function' and not '%RUNASSCRIPT' in buf[0] or conf.g.execplot==3:
...
def printdoc(projectname,projectdir,targetname,rebuildmode,args):
    ...
    conf.g.execplot=args.no_execplot
    conf.g.args=args
    do_execplot=args.no_execplot
    conf.g.rst_debug=args.rst_debug    
    ...
    nodocs_files = os.path.join(projectdir,'mat2doc','nodocs-files')
    if os.path.exists(nodocs_files):
        nodocslist = build_ignoredocrelist(projectdir, nodocs_files)
    else:
        nodocslist=[]
    
    ignore_folders = os.path.join(projectdir,'mat2doc','ignore-folders')
    if os.path.exists(ignore_folders):
        ignore_folders = safereadlines(ignore_folders)
    else:
        ignore_folders =[]
    conf.t.indexfiles=find_indexfiles(projectdir, ignore_folders )	# searches for path/path/Contents.m
    ...
    if conf.t.basetype=='php' or conf.t.basetype=='tex' or conf.t.basetype=='html':
        ...
        for fname in conf.t.indexfiles:
            ...
            for name in P.files:
                #print(fname)
                if not fname in conf.g.ignorelist:
        ...
        for fname in allfiles:
            ...
            #print('mach PHP allfiles: ', (tail+'.m'))
            if not (tail+'.m') in conf.g.ignorelist:
                if do_rebuild_file(sourcefile, targetfile, rebuildmode):
                    print(conf.t.basetype+' '+fname+'.m')
                    conf.execplot = do_execplot
                    P=matfile_factory(conf,fname, targfname)
                    #print(conf.execplot)
                    P.write_the_file()
            else:
                print('IGNORED --> '+conf.t.basetype+' '+fname)
    ...
    if conf.t.basetype=='mat':
        ...
        for root, dirs, files in os.walk(conf.t.dir, topdown=False):
            ...
            for mfile in [x for x in [x for x in files if x[-2:]=='.m'] if os.path.relpath(os.path.join(root, x),conf.t.dir) not in nodocslist]:					 
                ...
                if mfile in conf.g.ignorelist: # PM:10.5.2023: ignore if in the ignorelist
                    print('IGNORED --> MAT '+fullmfile)
                else:
                    print('MAT '+fullmfile)
...
description='mat2doc: Documentation generator adapted for the AMT.'
...
parser.add_argument('--rst-debug', action="store_true", default=False, help='Print reStructuredText input then terminate.')
...
printdoc(projectname,projectdir,args.target,rebuildmode,args)