...
%
%   Output parameters:
%      data  : Data for various conditions and listeners depending on
%              specified flag - see description below
%
...
%
%     'P3_data'  DTF and response data of listener P3 for all conditions as
%                described below. Naming of data fields: 
%
...
% of merchantability or fitness for a particular purpose. 
  ...
  definput.flags.type={...
      'missingflag','P3_data','P6_data',...
      'P3_b','P3_2o','P3_1ol','P3_1om','P3_1oh',...
      'P6_b','P6_2o','P6_1ol','P6_1om','P6_1oh',...
      'P3_dtf','P6_dtf','P3_dtf_bmp','P6_dtf_bmp'
                      };
  % Parse input options
  ...
  if flags.do_P3_data||flags.do_P6_data
    listener=varargin{1}(1:2);
    fname = [listener '_data'];
    data = amt_cache('get', fname, flags.cachemode);
    if isempty(data)
      amt_disp('Calculation of impulse responses');
      temp=data_langendijk2002([listener '_dtf']);
      fs=temp.stimPar.SamplingRate;           
      data.pol=temp.posLIN(:,5)';
      temp=temp.ampMdB;
      med=temp(1:end,:);
      amt_disp('  Condition: b');
      temp=data_langendijk2002([listener '_b']);
      data.targetb=temp(1,:); 
      data.responseb=temp(2,:);
      data.medir=gr2ir(med,'b',fs);
      amt_disp('  Condition: 2o');
      temp=data_langendijk2002([listener '_2o']);
      data.targetc=temp(1,:); 
      data.response2o=temp(2,:);
      data.medir2o=gr2ir(med,'2o',fs);
      amt_disp('  Condition: 1ol');
      temp=data_langendijk2002([listener '_1ol']);
      data.response1ol=temp(2,:);
      data.medir1ol=gr2ir(med,'1ol',fs);
      amt_disp('  Condition: 1om');
      temp=data_langendijk2002([listener '_1om']);
      data.response1om=temp(2,:);
      data.medir1om=gr2ir(med,'1om',fs);
      amt_disp('  Condition: 1oh');
      temp=data_langendijk2002([listener '_1oh']);
      data.response1oh=temp(2,:);
      data.medir1oh=gr2ir(med,'1oh',fs);
      data.fs = fs;
      amt_cache('set', fname, data);
    end
